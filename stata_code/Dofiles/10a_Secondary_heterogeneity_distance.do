...
*GC Year Fixed effects + cluster standard errors for selected:
...
esttab selected_heterog_quartiles prio_heterog_quartiles using "heterog_did.tex", replace label starlevels(* 0.1 ** 0.05 *** 0.01) se stats(N) title(Difference-in-differences by nearest_city) compress
