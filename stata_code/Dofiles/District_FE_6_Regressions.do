...
*GC Fixed effects + cluster standard errors 
reghdfe ndvi treatment post did if year != 2021, absorb(id) vce(cluster rnd_road_id) 

*GC Fixed effects + District FE
reghdfe ndvi treatment post did if year != 2021, absorb(id district) vce(cluster rnd_road_id) 

*GC Fixed effects + District FE
reghdfe ndvi treatment post did if year != 2021, absorb(year district) vce(cluster rnd_road_id) 

*** With rainfall ***

*GC Fixed effects + District FE + rain
reghdfe ndvi treatment post did rainfall if year != 2021, absorb(id district) vce(cluster rnd_road_id) 

*GC Fixed effects + District FE + rain + province
reghdfe ndvi treatment post did rainfall if year != 2021, absorb(year district) vce(cluster rnd_road_id) 

*GC Fixed effects + District FE + rain + province + ID
reghdfe ndvi treatment post did rainfall if year != 2021, absorb(id district province) vce(cluster rnd_road_id) 

*** Time-invariant controls ***, but province_cat instead of nampula
reghdfe ndvi treatment post did rainfall r_length_km population_2019 nearest_city if year != 2021, absorb(year district) vce(cluster rnd_road_id)


*** test areg***
areg ndvi treatment post did rainfall i.year if year != 2021, absorb(district) vce(cluster rnd_road_id) 

*** Time-invariant controls ***, but province_cat instead of nampula
areg ndvi treatment post did rainfall r_length_km population_2019 nearest_city i.year if year != 2021, absorb(district) vce(cluster rnd_road_id)
